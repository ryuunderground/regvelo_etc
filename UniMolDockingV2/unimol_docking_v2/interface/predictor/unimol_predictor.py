...
class UnimolPredictor:
    ...
    def postprocess(self, output_pkl, output_lmdb, output_ligand_name, output_ligand_dir, input_ligand, input_protein):
        ...
        output_ligand_sdf, predicted_rmsd = postprocessor.get_sdf(mol_list, smi_list, coords_predict_list, holo_center_coords_list, prmsd_score_list, output_ligand_name, output_ligand_dir, tta_times=self.conf_size)
        if self.steric_clash_fix:
            output_ligand_sdf = postprocessor.clash_fix(output_ligand_sdf, input_protein, input_ligand)
        return output_ligand_sdf, predicted_rmsd
    ...
    def predict_sdf(self, input_protein:str,
        ...
        output_pkl, output_lmdb = self.predict(input_protein,
                                               ...
                                               output_ligand_dir,
                                               batch_size)
        output_sdf, predicted_rmsd = self.postprocess(output_pkl,
                                      output_lmdb,
                                      output_ligand_name,
                                      output_ligand_dir,
                                      input_ligand,
        ...
        # return sdf path + the model's own predicted-RMSD confidence (lower = more confident)
        return input_protein, input_ligand, input_docking_grid, output_sdf, predicted_rmsd
