...
class Processor:
    ...
    def get_sdf(self, mol_list, smi_list, coords_predict_list, holo_center_coords_list, prmsd_score_list, output_ligand_name, output_ligand_dir, tta_times=10):
        ...
        output_ligand_list = []
        # ponytail: prmsd_score (the model's own predicted-RMSD confidence) was
        # computed to pick the best of tta_times candidate poses, then discarded.
        # Surface it instead -- both as an SDF property and a returned list.
        predicted_rmsd_list = []
        if self.mode == 'single':
        ...
        for i in tqdm(range(len(smi_list)//tta_times)):
            ...
            predicted_rmsd = float(prmsd_score_tta[idx])
            mol.SetProp("predicted_rmsd", f"{predicted_rmsd:.4f}")
            predicted_rmsd_list.append(predicted_rmsd)
        ...
        print("Done!")
        if self.mode == 'single':
            return output_ligand_list[0], predicted_rmsd_list[0]
        elif self.mode in ['batch_one2one', 'batch_one2many']:
            return output_ligand_list, predicted_rmsd_list
