...
def main(args):
    if args.mode == 'single':
        ...
        (input_protein,
         ...
         input_docking_grid,
         output_ligand,
         predicted_rmsd) = clf.predict_sdf(
        ...
        print('predicted RMSD (model self-confidence, lower = more confident):', predicted_rmsd)
        ...
        scores_path = os.path.join(args.output_ligand_dir, "predicted_rmsd.csv")
        pd.DataFrame({
            "output_ligand_name": output_ligand_name,
            "input_ligand": input_ligand,
            "output_ligand_sdf": output_ligand,
            "predicted_rmsd": predicted_rmsd,
        }).to_csv(scores_path, index=False)
        print("wrote confidence scores to", scores_path)
        end_time = time.time()
        execution_time = end_time - start_time
